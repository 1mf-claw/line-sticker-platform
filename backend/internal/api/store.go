...
type Store struct {
	...
	aiVerified map[string]map[string][]string
}
...
func NewStore() *Store {
	...
	s := &Store{db: db, ai: ai.MockPipeline{}, aiSecrets: map[string]AICredentialsRequest{}, aiVerified: map[string]map[string][]string{}}
	s.migrate()
	return s
}
